 ...
 */
public class DragDropSQL {
    
    ListView from, to;
    
    public static final String DEFAULT_ITEM = "Drag and drop SQL here...";

    public DragDropSQL(MainWindowController mw, ListView from, ListView to) {
        this.from = from;
        this.to = to;
        setup(mw);
    }
        ...
        to.setCellFactory(new Callback<ListView<String>, ListCell<String>>() {
            ...
            public ListCell<String> call(ListView<String> stringTreeView) {
                ...
                treeCell.setOnDragDropped((DragEvent event) -> {
                    ...
                    if (db.hasString()) {
                        try {
                            SetManager.getInstance().getSetByName(db.getString());
                        } catch (Exception e) {
